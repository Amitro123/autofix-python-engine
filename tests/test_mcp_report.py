...
def test_format_report_counts_error_state_separately_from_no_match(tmp_path):
    # resolved_by="error" (server.py's internal-failure state) must be
    # itemized on its own line, not folded into no_match or silently
    # dropped from the top-level breakdown while still counting toward
    # Total calls.
    log_path = tmp_path / "mcp_telemetry.jsonl"
    records = [
        {"ts": "t1", "error_type": "ImportError", "resolved_by": "fix", "input_chars": 10, "estimated_tokens_saved": 500},
        {"ts": "t2", "error_type": "UnknownError", "resolved_by": "error", "input_chars": 10, "estimated_tokens_saved": 0},
    ]
    log_path.write_text("\n".join(json.dumps(r) for r in records) + "\n", encoding="utf-8")
    ...
    assert "no_match: 0" in report
    assert "error: 1" in report
    assert "UnknownError: 1 error" in report
